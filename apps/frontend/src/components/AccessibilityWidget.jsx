          ...
          <motion.div initial={{opacity:0,y:20,scale:0.95}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:20,scale:0.95}} transition={{duration:0.2,ease:"easeOut"}}
            ...
            <div className="bg-dark text-footer-text p-4 flex items-center justify-between border-b border-white/10">
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-serif font-bold text-[#faf7f4] text-sm">Accessibility</span>
              </div>
              <button onClick={() => setOpen(false)} aria-label="Close accessibility panel" className="text-footer-text/70 hover:text-white p-1.5 rounded-full hover:bg-white/10 transition-colors"><X size={15} /></button>
            </div>
            <div className="p-4 space-y-4">
              <div className="flex items-center justify-between gap-3 p-3 rounded-2xl bg-surface dark:bg-stone-900 border border-stone-200/60 dark:border-stone-800">
                <div className="flex items-center gap-2.5">
                  ...
                  <div><p className="text-xs font-bold text-dark dark:text-stone-100 leading-none">High Contrast</p><p className="text-[10px] text-muted-foreground mt-0.5">Boost text visibility</p></div>
                </div>
                <button role="switch" aria-checked={hc} onClick={() => setContrast(!hc)}
                  ...
                  <span className="sr-only">{hc ? "Disable" : "Enable"} high contrast</span>
                </button>
              </div>
              <div className="p-3 rounded-2xl bg-surface dark:bg-stone-900 border border-stone-200/60 dark:border-stone-800 space-y-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div><p className="text-xs font-bold text-dark dark:text-stone-100 leading-none">Text Size</p><p className="text-[10px] text-muted-foreground mt-0.5">Current: {FONT_LABELS[FONT_STEPS[fs]]}</p></div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => fs > 0 && setFont(fs - 1)} disabled={fs === 0} aria-label="Decrease font size"
                  ...
                  <div className="flex-1 flex items-center justify-center gap-1.5">
                    {FONT_STEPS.map((_, i) => (
                      <button key={i} onClick={() => setFont(i)} aria-label={"Set text size " + FONT_LABELS[FONT_STEPS[i]]}
                  ...
                  <button onClick={() => fs < FONT_STEPS.length - 1 && setFont(fs + 1)} disabled={fs === FONT_STEPS.length - 1} aria-label="Increase font size"
              ...
              {hasChanges && <button onClick={reset} className="w-full text-xs text-muted-foreground hover:text-primary dark:hover:text-amber-300 transition-colors py-1 font-bold uppercase tracking-wider">Reset to defaults</button>}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      <motion.button whileHover={{scale:1.08}} whileTap={{scale:0.94}} onClick={() => setOpen(v => !v)}
        aria-label="Accessibility options" aria-expanded={open} title="Accessibility options"
