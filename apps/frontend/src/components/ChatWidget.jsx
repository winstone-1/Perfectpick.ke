...
const ChatWidget = () => {
  ...
  const send = (e) => {
    ...
    window.open("https://wa.me/254787251690?text=" + encodeURIComponent("Hi PerfectPick Support, " + message), "_blank", "noopener,noreferrer");
    setMessage("");
  };
  return (
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{opacity:0,y:30,scale:0.92}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:30,scale:0.92}} transition={{duration:0.25,ease:"easeOut"}}
            className="w-[300px] sm:w-[340px] bg-card text-card-foreground rounded-3xl shadow-[0_16px_50px_rgba(0,0,0,0.22)] overflow-hidden border border-stone-200/80 dark:border-stone-800">
