...
const Home = () => {
  ...
  const features = [
    { Icon: FaStar,          title: 'Premium Quality',  desc: 'Only the best for our picks'      },
    { Icon: FaTruck,         title: 'Lipa na M-Pesa',   desc: 'Fast & Secure checkout'           },
    { Icon: FaShield,        title: 'Genuine Items',    desc: '100% authenticity guaranteed'     },
    { Icon: FaArrowsRotate,  title: 'Easy Returns',     desc: 'Stress-free experience'           },
  ];
        ...
        <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 items-center gap-12 relative z-10">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="space-y-8">
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/products"><Button className="btn-primary h-14 px-8 text-lg">Shop Now</Button></Link>
              <Link to="/about"><Button variant="outline" className="btn-outline h-14 px-8 text-lg">Our Story</Button></Link>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }} className="grid grid-cols-2 gap-4">
            {[FaBagShopping, FaShoePrints, FaGem, FaShirt].map((Icon, i) => (
              <motion.div key={i} whileHover={{ scale: 1.05, rotate: i % 2 === 0 ? 2 : -2 }}
                className="aspect-square glass rounded-3xl flex items-center justify-center shadow-xl text-primary"
              >
                <Icon size={80} />
              </motion.div>
            ))}
        ...
        <div className="bg-dark text-footer-text rounded-3xl p-8 md:p-12 shadow-2xl flex flex-col md:flex-row justify-around items-center gap-8 border border-white/10">
          {stats.map((stat, i) => (
            <div key={i} className="text-center md:border-r last:border-0 border-white/20 pr-0 md:pr-12 w-full md:w-auto last:pr-0">
              <div className="text-3xl font-serif font-bold text-primary flex items-center justify-center gap-2">
                {stat.value}
                {stat.Icon && <stat.Icon size={20} className="text-amber-400" />}
              </div>
            ...
            <motion.div key={i} variants={itemVariants}>
              <Link to={href} className="group block h-full">
                <div className="h-full bg-white rounded-2xl p-6 text-center border border-transparent hover:border-primary/20 hover:shadow-xl transition-all duration-300 flex flex-col items-center gap-4">
                  <div className="group-hover:scale-125 transition-transform duration-300 text-primary">
                    <Icon size={36} />
                  </div>
                  <div>
      ...
      <section className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map(({ Icon, title, desc }, i) => (
            <motion.div key={i} whileHover={{ y: -5 }} className="p-6 rounded-2xl bg-white border border-border/10 shadow-sm text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="bg-white/5 backdrop-blur-sm p-8 rounded-3xl border border-white/10 text-left space-y-4">
                ...
                <p className="text-footer-text/80 italic leading-relaxed">"{text}"</p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold">
                    {name[0]}
                  </div>
                  <div>
                    <p className="text-footer-text font-bold text-sm">{name}</p>
